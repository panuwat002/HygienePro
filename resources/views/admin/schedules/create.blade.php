<x-app-layout>
    @section('header', 'Create Inspection Schedule')
                ...
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="{{ route('schedules.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" id="title" class="form-control" value="{{ old('title') }}" required placeholder="e.g. ตรวจเช้า (Morning Check)">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="mt-1">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="target_type" id="type_location" value="location" onchange="toggleTargetSelect()" {{ old('target_type', 'location') == 'location' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="type_location">จุดประจำการ (Location/Area)</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="target_type" id="type_machine" value="machine" onchange="toggleTargetSelect()" {{ old('target_type') == 'machine' ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="target_id" id="target_id_location" class="form-select">
                                ...
                                @foreach($locations as $location)
                                    <option value="{{ $location->id }}" {{ old('target_id') == $location->id ? 'selected' : '' }}>{{ $location->location_name }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="target_id" id="target_id_machine" class="form-select d-none" disabled>
                                ...
                                @foreach($machines as $machine)
                                    <option value="{{ $machine->id }}" {{ old('target_id') == $machine->id ? 'selected' : '' }}>{{ $machine->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="frequency" id="frequency" class="form-select" onchange="toggleDays()" required>
                                <option value="daily" {{ old('frequency') == 'daily' ? 'selected' : '' }}>ทุกวัน (Daily)</option>
                                <option value="weekly" {{ old('frequency') == 'weekly' ? 'selected' : '' }}>รายสัปดาห์ (Weekly)</option>
                            </select>
                        </div>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-text text-muted small">* กำหนดช่วงเวลาที่ต้องตรวจให้เสร็จสิ้น</div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-dark">
                                <i class="bi bi-save me-1"></i> บันทึกข้อมูล (Save Schedule)
                            </button>
                        </div>
                    </form>

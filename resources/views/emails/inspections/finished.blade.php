...
<body style="font-family: sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 15px; border-radius: 5px; margin: 20px 0;">
        ...
        <ul style="list-style: none; padding: 0; margin: 0;">
            <li style="margin-bottom: 8px;"><strong>จำนวนที่ตรวจทั้งหมด:</strong> {{ $stats['total_targets'] ?? 0 }} {{ $stats['unit'] ?? 'รายการ' }} (รวม {{ $stats['total'] }} จุดตรวจ)</li>
            <li style="margin-bottom: 8px; color: #059669;"><strong>ผ่าน (Pass):</strong> {{ $stats['passed_targets'] ?? 0 }} {{ $stats['unit'] ?? 'รายการ' }} (รวม {{ $stats['pass'] }} จุดตรวจ)</li>
            <li style="color: #dc2626;"><strong>ไม่ผ่าน (Fail):</strong> {{ $stats['failed_targets'] ?? 0 }} {{ $stats['unit'] ?? 'รายการ' }} (พบปัญหา {{ $stats['fail'] }} จุดตรวจ)</li>
        </ul>
    </div>
